...
import {ProfileBtn, __RewireAPI__ as P} from '@components/ProfileBtn'
import {shallow} from 'enzyme'

describe('The ProfileBtn Component', () => {
  describe('The Renderer', () => {
    it(`renders the profile button`, () => {
      P.__set__('getStatus', () => ({firstName: 'Joe', surname: 'Blow', email: 'joe'}))
      const wrapper = shallow(<ProfileBtn />)
      expect(wrapper.find('#ProfileBtn')).toHaveLength(1)
      expect(wrapper.find('#ProfileBtn > span.button')).toHaveLength(1)
      expect(wrapper.find('#ProfileBtn span.icon')).toHaveLength(1)
    })
